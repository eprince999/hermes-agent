...
Stage 3 (Cursor API + speaker): put `CURSOR_API_KEY=crsr_...` in `~/lelamp_runtime/.env` (key from https://cursor.com/dashboard/api). `uv add cursor-sdk piper-tts`, then `sudo uv run python local_main.py --download-piper` (Ryan neural English, streamed so speech starts immediately). `espeak-ng` is a robotic fallback only. Test with `--listen` for back-and-forth, or `--sim --ask "Do you agree warm light is nicer?"` and `--speak "Hey. I'm here with you."`. Nod plays while the spoken line is generated. Short keywords run locally. Full sentences go to Cursor. `--no-speak` prints only. This is cursor-sdk, not a DeepSeek/OpenAI chat URL. Do not clone celebrity or TV voices; a recording of the user's own voice can be a later opt-in.

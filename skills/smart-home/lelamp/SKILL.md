...
Stage 4 (Vosk + music dance): put `CURSOR_API_KEY=crsr_...` in `~/lelamp_runtime/.env`, `uv add vosk cursor-sdk`, then `--download-vosk` and `--listen`. Voice **desk commands run locally and skip the wake word**: `lights on` / `lights off`, `brighter` / `dimmer`, `study mode` (white), `reading mode` (yellow, lean to the book), `yellow light`, `white light`, `closer` / `look down`, **`music` / `dance`** (random beat + twist on the rhythm), **`stop music`**. Extra wav/mp3 files can go in `~/lelamp_runtime/music/`. Other talk is coin-flipped to Cursor, which may play one official recording (`nod`, `headshake`, `curious`, …) and never speaks. Camera stills are on-demand (`look` / `--snap`). Test with `--sim --no-cursor --say "music"`. Snapshot this file with `--snapshot` (writes `lamp_snapshots/stage4.py`).
